...
def test_export_students_excel_empty_class(app):
    """تصدير قائمة طلاب فارغة."""
    from app.services.export import export_students_excel
    ...
    with app.app_context():
        result = export_students_excel(class_id)
        assert isinstance(result, bytes)
        # Should still produce a valid file (header only)
        assert len(result) > 0


def test_export_grades_excel_no_items(app):
    """تصدير درجات صف بلا عناصر درجات."""
    from app.services.export import export_grades_excel
...
def test_export_progress_excel_all_statuses(app):
    """تصدير تقدم مع حالات مختلفة."""
    from app.services.export import export_progress_excel
    ...
    with app.app_context():
        lesson_id = make_lesson(app, class_id)
        sid1 = make_user(app, "student", school_id=school_id)
        sid2 = make_user(app, "student", school_id=school_id)
        # Use valid statuses (completed fits VARCHAR(10))
        make_student_progress(app, sid1, lesson_id, class_id, status="completed", pct=100, seconds=600)
        make_student_progress(app, sid2, lesson_id, class_id, status="completed", pct=50, seconds=300)
        result = export_progress_excel(class_id)
        assert isinstance(result, bytes)
        assert len(result) > 0


def test_export_moe_format(app):
    """تصدير بيانات بصيغة الوزارة."""
    ...
    with app.app_context():
        ...
        item_id = make_grade_item(app, class_id, cat_id, "امتحان", 20)
        make_grade_entry(app, student_id, item_id, 18)
        result = export_moe_format(school_id=school_id)
        assert isinstance(result, bytes)
        assert len(result) > 0


def test_export_moe_format_empty(app):
    """تصدير بيانات الوزارة مع لا صفوف."""
    from app.services.export import export_moe_format
