...
class TestSchoolsService:
    ...
    def test_join_class(self, app):
        ...
        with app.app_context():
            ...
            _db.session.commit()
            student = _user(app, "student")
            err = join_class(c_obj, _db.session.get(User, student))
            assert err is None
            assert is_member(c_obj, _db.session.get(User, student)) is True
    ...
    def test_join_class_already_member(self, app):
        ...
        with app.app_context():
            ...
            _db.session.commit()
            student = _user(app, "student")
            join_class(c_obj, _db.session.get(User, student))
            err = join_class(c_obj, _db.session.get(User, student))
    ...
    def test_join_class_full(self, app):
        ...
        with app.app_context():
            ...
            join_class(c_obj, _db.session.get(User, s1))
            s2 = _user(app, "student")
            err = join_class(c_obj, _db.session.get(User, s2))
            assert "ممتلئ" in err
...
class TestContentService:
    ...
    def test_delete_attachment(self, app):
        ...
        with app.app_context():
            ...
            assert _db.session.get(LessonAttachment, att.id) is None
